...
function AdminPage() {
  const query = useQuery({ queryKey: queryKeys.kycApplications, queryFn: fetchKycApplications });
  ...
  const table = useReactTable({
    columns: defaultColumns, data: query.data ?? [],
      ...
      <Table className="w-40">
        <TableCaption>
          {query.isLoading
            ...
            : query.isError ? 'Failed to load KYC applications.' : ''}
        </TableCaption>
        <TableHeader>
        ...
        <TableBody>
          {query.isLoading ? (
            ...
            <TableRow>
              <TableCell colSpan={4}>Failed to load KYC applications.</TableCell>
            </TableRow>
